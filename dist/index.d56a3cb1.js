...
const startRafLoop = (runEveryFrame)=>{
    ...
    const performRafTick = ()=>{
        ...
        for(var i = 0; i < 1000; i++)runEveryFrame();
    };
    performRafTick();
...
const drawPolygon = (vertices)=>{
    ...
    for(var i = 1; i < vertices.length; i++)ctx.lineTo(vertices[i][0], vertices[i][1]);
    ctx.closePath();
};
const translate = (from, to)=>[
        from[0] + to[0],
        from[1] + to[1]
    ]
;
const scale = (from, factor)=>[
        from[0] * factor,
        from[1] * factor
    ]
;
const drawPanel = (vertices, color, x, y, size)=>{
    ctx.fillStyle = color.mix("#b6cee2", (1 - y / window.innerHeight) * 0.5).alpha((random() ** 2 + 0.4) * 0.8);
    drawPolygon(vertices.map((from)=>translate(scale(from, y / window.innerHeight * 4 * size), [
            x,
            y - HORIZON, 
        ])
    ));
...
const drawBlock = ({ x , y , rotation , height , size  })=>{
    let baseColor = _chromaJsDefault.default("#dcc6b2").mix("#4f6584", random());
    //   if (chance(0.8)) {
...
const addNoise = ()=>{
    ...
    ctx.fillStyle = _chromaJsDefault.default("#b6cee2").alpha(random(0.2));
    ctx.fillRect(random(window.innerWidth), random(window.innerHeight), 1, 1);
};
let y1 = HORIZON;
const tick = ()=>{
    y1 += (y1 / window.innerHeight) ** 2 / (window.innerWidth / 2000);
    drawBuilding({
        x: random(window.innerWidth),
        y: y1
    });
    if (y1 > window.innerHeight + HORIZON + 500) // for (var i = 0; i < 1000; i++) {
    //   addNoise();
    // }
    stopLoop();
};
...
if (module.hot) module.hot.dispose(destroy);

},{"chroma-js":"1Jlhj","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"1Jlhj":[function(require,module,exports) {
(function(global, factory) {
    ...
    var clip_rgb = function(rgb) {
        ...
        for(var i = 0; i <= 3; i++){
            ...
            } else if (i === 3) rgb[i] = limit(rgb[i], 0, 1);
        }
        return rgb;
    };
    ...
    var unpack = function(args, keyOrder) {
        ...
        if (type(args[0]) == 'object' && keyOrder) return keyOrder.split('').filter(function(k) {
    ...
    var Color = function Color() {
        ...
        var me = this;
        if (type$1(args[0]) === 'object' && args[0].constructor && args[0].constructor === this.constructor) // the argument is already a Color instance
        return args[0];
    ...
    var chroma = function() {
        var args = [], len = arguments.length;
        while(len--)args[len] = arguments[len];
        return new (Function.prototype.bind.apply(chroma.Color, [
            null
        ].concat(args)));
    };
    ...
    chroma_1.cmyk = function() {
        var args = [], len = arguments.length;
        while(len--)args[len] = arguments[len];
        return new (Function.prototype.bind.apply(Color_1, [
            null
        ].concat(args, [
            'cmyk'
        ])));
    ...
    input.autodetect.push({
        ...
        test: function() {
            ...
            if (type$2(args) === 'array' && args.length === 4) return 'cmyk';
        }
    });
     ...
     */ var rgb2css = function() {
        ...
        if (mode.substr(0, 3) == 'hsl') return hsl2css_1(rgb2hsl_1(rgba), mode);
        rgba[0] = round(rgba[0]);
        rgba[1] = round(rgba[1]);
        rgba[2] = round(rgba[2]);
    ...
    var css2rgb = function(css) {
        ...
        if (m = css.match(RE_RGB_PCT)) {
            ...
            for(var i$2 = 0; i$2 < 3; i$2++)rgb$2[i$2] = round$2(rgb$2[i$2] * 2.55);
        ...
        if (m = css.match(RE_RGBA_PCT)) {
            ...
            for(var i$3 = 0; i$3 < 3; i$3++)rgb$3[i$3] = round$2(rgb$3[i$3] * 2.55);
            rgb$3[3] = +rgb$3[3];
            return rgb$3;
        }
        // hsl(0,100%,50%)
    ...
    input.autodetect.push({
        ...
        test: function(h) {
            ...
            if (!rest.length && type$3(h) === 'string' && css2rgb_1.test(h)) return 'css';
        }
    });
    ...
    input.autodetect.push({
        ...
        test: function() {
            ...
            if (type$4(args) === 'array' && args.length === 3) return 'hcg';
        }
    });
    ...
    input.autodetect.push({
        ...
        test: function(h) {
            var rest = [], len = arguments.length - 1;
            while(len-- > 0)rest[len] = arguments[len + 1];
            if (!rest.length && type$5(h) === 'string' && [
                3,
                4,
                5,
                6,
    ...
    input.autodetect.push({
        ...
        test: function() {
            ...
            if (type$6(args) === 'array' && args.length === 3) return 'hsi';
        }
    });
    ...
    input.autodetect.push({
        ...
        test: function() {
            ...
            if (type$7(args) === 'array' && args.length === 3) return 'hsl';
        }
    });
    ...
    input.autodetect.push({
        ...
        test: function() {
            ...
            if (type$8(args) === 'array' && args.length === 3) return 'hsv';
        }
    });
    var labConstants = {
    ...
    input.autodetect.push({
        ...
        test: function() {
            ...
            if (type$9(args) === 'array' && args.length === 3) return 'lab';
        }
    });
    ...
    [
        ...
        return input.autodetect.push({
            ...
            test: function() {
                ...
                if (type$a(args) === 'array' && args.length === 3) return m;
            }
        });
    });
    /**
    ...
    */ var w3cx11 = {
    ...
    input.autodetect.push({
        ...
        test: function(h) {
            var rest = [], len = arguments.length - 1;
            while(len-- > 0)rest[len] = arguments[len + 1];
            if (!rest.length && type$b(h) === 'string' && w3cx11_1[h.toLowerCase()]) return 'named';
        }
    });
    ...
    input.autodetect.push({
        ...
        test: function() {
            var args = [], len = arguments.length;
            while(len--)args[len] = arguments[len];
            if (args.length === 1 && type$d(args[0]) === 'number' && args[0] >= 0 && args[0] <= 16777215) return 'num';
        }
    });
    ...
    input.autodetect.push({
        ...
        test: function() {
            ...
            if (type$e(args) === 'array' && (args.length === 3 || args.length === 4 && type$e(args[3]) == 'number' && args[3] >= 0 && args[3] <= 1)) return 'rgb';
        }
    });
    /*
    ...
    Color_1.prototype.luminance = function(lum) {
        if (lum !== undefined && type$g(lum) === 'number') {
            ...
            var rgb = (cur_lum > lum ? test(new Color_1([
                0,
                0,
                0
            ]), this) : test(this, new Color_1([
                255,
                255,
                255
            ]))).rgb();
            return new Color_1(rgb.concat([
                this._rgb[3]
            ]));
        }
    ...
    Color_1.prototype.set = function(mc, value, mutate) {
        ...
        if (channel) {
            var i = mode.indexOf(channel);
            if (i > -1) {
                if (type$i(value) == 'string') switch(value.charAt(0)){
                    case '+':
                        src[i] += +value;
                        break;
                    case '-':
                ...
                else if (type$i(value) === 'number') src[i] = value;
    ...
    var hsl$11 = function(col1, col2, f) {
        return _hsx(col1, col2, f, 'hsl');
    };
    // register interpolator
    interpolator.hsl = hsl$11;
    ...
    var scale = function(colors1) {
        ...
        var setColors = function(colors) {
            ...
            if (colors && type$j(colors) === 'string' && chroma_1.brewer && chroma_1.brewer[colors.toLowerCase()]) colors = chroma_1.brewer[colors.toLowerCase()];
            if (type$j(colors) === 'array') {
                ...
                for(var c = 0; c < colors.length; c++)colors[c] = chroma_1(colors[c]);
                // auto-fill color position
                _pos1.length = 0;
                for(var c$1 = 0; c$1 < colors.length; c$1++)_pos1.push(c$1 / (colors.length - 1));
            }
            resetCache();
        ...
        var getColor = function(val, bypassMap) {
            ...
            if (_useCache && _colorCache[k]) col = _colorCache[k];
                ...
                if (type$j(_colors) === 'array') //for i in [0.._pos.length-1]
                for(var i = 0; i < _pos1.length; i++){
                    var p = _pos1[i];
                    if (t <= p) {
                        col = _colors[i];
                        break;
                    }
                    if (t >= p && i === _pos1.length - 1) {
                        col = _colors[i];
                        break;
                    }
                    if (t > p && t < _pos1[i + 1]) {
                ...
                else if (type$j(_colors) === 'function') col = _colors(t);
        ...
        f1.classes = function(classes) {
            if (classes != null) {
                if (type$j(classes) === 'array') {
                    ...
                    else _classes = chroma_1.limits(d, 'e', classes);
                }
                return f1;
            }
            return _classes;
        ...
        f1.domain = function(domain) {
            ...
            for(var i3 = 0, list = Array.from(domain); i3 < list.length; i3 += 1){
                var d = list[i3];
                _pos1.push((d - _min) / (_max - _min));
            }
            else {
                for(var c = 0; c < k; c++)_pos1.push(c / (k - 1));
                if (domain.length > 2) {
                    // set domain map
        ...
        f1.correctLightness = function(v) {
            ...
            if (_correctLightness) tMapLightness = function(t) {
                ...
                var t1 = 1;
                var max_iter = 20;
                while(Math.abs(L_diff) > 0.01 && max_iter-- > 0)(function() {
                    if (pol) L_diff *= -1;
                    if (L_diff < 0) {
                        t0 = t;
                        t += (t1 - t) * 0.5;
        ...
        f1.colors = function(numColors, out) {
            ...
            if (chroma_1[out]) result = result.map(function(c) {
                return c[out]();
            });
            return result;
        };
    ...
    var cubehelix = function(start, rotations, hue, gamma, lightness) {
        ...
        var dh = 0, dl;
        if (type$k(lightness) === 'array') dl = lightness[1] - lightness[0];
        else {
            dl = 0;
            lightness = [
                lightness,
    ...
    var random_1 = function() {
        var code = '#';
        for(var i = 0; i < 6; i++)code += digits.charAt(floor$2(random() * 16));
    ...
    var limits1 = function(data, mode, num) {
        ...
        if (mode.substr(0, 1) === 'e') {
            ...
            limits.push(min);
            for(var i$1 = 1; i$1 < num; i$1++)limits.push(pow$7(10, min_log + i$1 / num * (max_log - min_log)));
            limits.push(max);
    ...
    var deltaE = function(a, b, L, C) {
        ...
        var h1 = c1 < 0.000001 ? 0 : atan2$2(b1, a1) * 180 / PI$2;
        while(h1 < 0)h1 += 360;
        while(h1 >= 360)h1 -= 360;
    ...
    var valid = function() {
        ...
        while(len--)args[len] = arguments[len];
        try {
            new (Function.prototype.bind.apply(Color_1, [
                null
            ].concat(args)));
            return true;
        } catch (e) {
    ...
    */ var colorbrewer = {
    ...
    for(var i$11 = 0, list$1 = Object.keys(colorbrewer); i$11 < list$1.length; i$11 += 1){
        var key1 = list$1[i$11];
...
},{}]},["1Mq12","5HwUs"], "5HwUs", "parcelRequiref558")
